...
const QueryResponseForm = ({ onEdit, onDelete, userRole, userId }) => {
  ...
  // Function to fetch query response records with pagination, sorting, and filtering
  const fetchResponseRecords = async (currentPage, sortKey, sortDirection, filterParams) => {
    setLoading(true);
    try {
      const params = {
        page: currentPage,
      ...
      // Check user role to filter responses based on role
      if (userRole === 'query_manager') {
        params.query_manager_id = userId; // Only fetch responses by this user
      }

      const response = await api.get('/responses', { params });
      setResponseRecords(response.data.records); // Assume the API returns response records in "records"
  ...
  const debouncedFetchResponses = useCallback(debounce(fetchResponseRecords, 300), []);
  ...
  const handleSort = (key) => {
    ...
    debouncedFetchResponses(page, key, direction, filters); // Pass filters to API call
  };

  // Handle page changes in pagination
  const handlePageChange = (pageNumber) => {
    setPage(pageNumber);
    fetchResponseRecords(pageNumber, sortConfig.key, sortConfig.direction, filters); // Fetch records for the selected page with filters
  };

  // Handle date range and client name filter submission
  const handleFilterSubmit = (e) => {
    e.preventDefault();
    fetchResponseRecords(page, sortConfig.key, sortConfig.direction, filters); // Fetch records with filters
  };
  ...
  useEffect(() => {
    fetchResponseRecords(page, sortConfig.key, sortConfig.direction, filters);
  }, [page, sortConfig]);
      ...
      <Table striped bordered hover responsive className="mt-3">
        <thead>
          <tr>
            ...
            <th onClick={() => handleSort('response_text')}>
              Response Text {getSortIcon('response_text')}
            </th>
            <th onClick={() => handleSort('response_status')}>
              Response Status {getSortIcon('response_status')}
            </th>
            <th onClick={() => handleSort('date_of_response')}>
              Date of Response {getSortIcon('date_of_response')}
            </th>
            <th>Actions</th>
          </tr>
        </thead>
        <tbody>
          {responseRecords.length > 0 ? (
            responseRecords.map((response, index) => (
              <tr key={response.id}>
                <td>{(page - 1) * limit + index + 1}</td> {/* Adjust for pagination */}
                <td>{response.client_name}</td>
                <td>{response.client_id_no}</td>
                <td>{response.query_type}</td>
                <td>{response.response_text}</td>
                <td>{response.response_status}</td>
                <td>{new Date(response.date_of_response).toLocaleDateString()}</td>
                <td>
                  <Button
                    variant="warning"
                    size="sm"
