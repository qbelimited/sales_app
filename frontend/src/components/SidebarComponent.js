...
// Ensure correct imports for Navbar and Sidebar
import Navbar from './Navbar';  // Assuming default export
import Sidebar from './Sidebar';  // Assuming default export

// Ensure correct imports for all pages
...
import Toaster from './Toaster';  // Ensure this is properly exported
...
function SidebarComponent() {
  ...
  useEffect(() => {
    ...
    if (savedRole && authService.isLoggedIn()) {
      ...
      fetchUserAccess(savedRole);  // Fetch access based on role
    } else {
      setRole(null);
      localStorage.removeItem('userRole');
      navigate('/login');
    }
  }, [navigate, fetchUserAccess]);  // Added fetchUserAccess to dependencies
